...
*/
{
  double *a;		/* Matriz colapsada em vector*/
  double *a_row;	/* Linha da matriz da matriz a */
  int m;		/* Numero de linhas da Matriz a */
  int n;		/* Numero de colunas da matriz */

  double ans;		/* Resultado da multiplicacao a_row[i] * x[i] */
  double *b;		/* Vetor resultado */
  int dest;		/* ID de um processo */
  int dummy;
  int ierr;		/* código para guardar valor de retorno das funções MPI */
  int j_one;
  
  int i,j;		/* indices */
  int k;
  
  int master = 0;	/* Process ID do MASTER */
  int my_id;		/* Process id */
  int num_procs;	/* Numero de processos */
  int num_rows;		
  int num_workers;
  double pi = 3.141592653589793;
  MPI_Status status;
  int tag;		/* Marca a linha em que o processo vai ter que começar a correr */
  int tag_done;		
  double *x;		/* Vector */
/*
  Initialize MPI.
...
*/
  else
  {
    for ( ; ; )
    {
      ...
      printf("Processo %d a executar linha %d\n",my_id,tag);
